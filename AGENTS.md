...
- **Test-FIRST, always — design the test before the code.** For *every* problem (bug or feature), the FIRST move is to write the test(s) that fail *because* the problem exists / the feature is missing. Watch them go **red** (they prove the gap and are the executable spec of "done"), write the minimum code to make them **green**, then **refactor**. Those tests then permanently guard delivery. Run the full suite before and after every change. Infra: `cargo test` (Rust), `vitest` (TS).
  ...
  **Edge-case checklist — run through this for every feature before writing code:**
  1. **Zero / empty** — what happens with no data? (empty buffer, zero population, no plants — the case most often skipped)
  2. **Data integrity** — assert the *values*, not just the shape. Asserting `count == 2` after a round-trip is weak; assert `density == 0.75` and `plant_type == 3`. Count-only tests mask corruption.
  3. **Consumer boundary** — test what the *downstream reader* receives, not just what the producer wrote. If Rust writes a buffer that JS reads, a test proving the buffer is populated is stronger than a test proving the Rust struct was populated.
  4. **Format / backward compat** — any time a serialized format gains a new field, prove that old files (missing the field) still deserialise cleanly. Use `#[serde(default)]` *and* write the test that would catch a missing default.
