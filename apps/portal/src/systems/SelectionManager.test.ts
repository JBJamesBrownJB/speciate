...
describe('SelectionManager', () => {
  ...
  describe('findNearestCreature', () => {
    ...
    it('should find nearest creature within click radius', () => {
      const result = selectionManager.findNearestCreature(creatures, 102, 98, 20);

      expect(result?.id).toBe(1);
    });

    it('should return null if no creature within radius', () => {
      const result = selectionManager.findNearestCreature(creatures, 500, 500, 20);

      expect(result).toBeNull();
    });

    it('should find the closest when multiple creatures in radius', () => {
      const result = selectionManager.findNearestCreature(creatures, 145, 145, 50);
    ...
    it('should account for creature size in hit detection', () => {
      ...
      const result = selectionManager.findNearestCreature(largeCreature, 115, 100, 5);

      expect(result?.id).toBe(1);
    });

    it('should return null for empty creature array', () => {
      const result = selectionManager.findNearestCreature([], 100, 100, 20);
  ...
  describe('updateSelectedPosition', () => {
    it('should update position of selected creature from buffer', () => {
      const creature: CreatureData = { id: 1, x: 100, y: 100, rotation: 0, size: 2 };
      selectionManager.selectCreature(creature);

      const updatedCreatures: CreatureData[] = [
        { id: 1, x: 150, y: 200, rotation: 1.5, size: 2 },
      ];

      selectionManager.updateSelectedFromBuffer(updatedCreatures);
    ...
    it('should deselect if creature no longer in buffer', () => {
      const creature: CreatureData = { id: 1, x: 100, y: 100, rotation: 0, size: 2 };
      selectionManager.selectCreature(creature);
