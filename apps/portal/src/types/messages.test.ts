...
describe('messages', () => {
  ...
  describe('adaptSimulationFrame', () => {
    ...
    it('should set default width and height for all creatures', () => {
      ...
      expect(result.creatures[0].width).toBe(1);
      expect(result.creatures[0].height).toBe(1);
      expect(result.creatures[1].width).toBe(1);
      expect(result.creatures[1].height).toBe(1);
    });
