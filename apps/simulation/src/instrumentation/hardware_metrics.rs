...
impl HardwareMetrics {
    ...
    pub fn read(&mut self) -> Option<HardwareSnapshot> {
        ...
        let cycles = self.cycles.as_mut().and_then(|c| c.read().ok()).unwrap_or(0);
        let instructions = self.instructions.as_mut().and_then(|c| c.read().ok()).unwrap_or(0);
        ...
        let l1d_miss_rate = if cache_refs_delta > 0 {
            (l1d_misses_delta as f64 / cache_refs_delta as f64) * 100.0
        } else {
            0.0
        };

        let l1i_miss_rate = if cache_refs_delta > 0 {
            (l1i_misses_delta as f64 / cache_refs_delta as f64) * 100.0
        } else {
            0.0
        };
    ...
    pub fn enable(&mut self) {
        if let Some(ref mut c) = self.cycles { c.enable().ok(); }
        if let Some(ref mut c) = self.instructions { c.enable().ok(); }
    ...
    pub fn disable(&mut self) {
        if let Some(ref mut c) = self.cycles { c.disable().ok(); }
        if let Some(ref mut c) = self.instructions { c.disable().ok(); }
