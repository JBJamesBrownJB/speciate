...
mod tests {
    ...
    #[test]
    fn test_save_state_preserves_update_slice() {
        use crate::simulation::core::components::Position;
        use crate::simulation::creatures::components::UpdateSlice;
        use crate::simulation::creatures::constants::UPDATE_SLICE_COUNT;
        use bevy_ecs::query::QueryState;

        let mut sim = SimulationBuilder::new().build();

        // Spawn creatures with different slice IDs (deterministic assignment: id % UPDATE_SLICE_COUNT)
        let id1 = sim.spawn_crit(CritBuilder::new().at(0.0, 0.0));
        let id2 = sim.spawn_crit(CritBuilder::new().at(10.0, 0.0));

        // Get original slice values (keyed by position for easy matching after restore)
        let original_slices: Vec<(f32, u8)> = {
            let mut query: QueryState<(&Position, &UpdateSlice)> = sim.world_mut().query();
            query
                .iter(sim.world())
                .map(|(pos, slice)| (pos.x, slice.id))
                .collect()
        };

        assert_eq!(original_slices.len(), 2);

        // Verify deterministic assignment
        let slice1 = original_slices.iter().find(|(x, _)| *x == 0.0).unwrap().1;
        let slice2 = original_slices.iter().find(|(x, _)| *x == 10.0).unwrap().1;
        assert_eq!(slice1, (id1 % UPDATE_SLICE_COUNT as u32) as u8);
        assert_eq!(slice2, (id2 % UPDATE_SLICE_COUNT as u32) as u8);

        // Save and restore
        let save_state = sim.to_save_state().expect("Failed to create save state");
        let mut restored_sim = Simulation::from_save_state(save_state)
            .expect("Failed to restore from save state");

        // Verify slice IDs are preserved (match by position)
        let restored_slices: Vec<(f32, u8)> = {
            let mut query: QueryState<(&Position, &UpdateSlice)> = restored_sim.world_mut().query();
            query
        ...
        assert_eq!(restored_slices.len(), 2, "Should restore 2 creatures");

        let restored_slice1 = restored_slices.iter().find(|(x, _)| *x == 0.0).unwrap().1;
        let restored_slice2 = restored_slices.iter().find(|(x, _)| *x == 10.0).unwrap().1;

        assert_eq!(restored_slice1, slice1, "UpdateSlice should be preserved for creature at x=0");
        assert_eq!(restored_slice2, slice2, "UpdateSlice should be preserved for creature at x=10");
    }
}
