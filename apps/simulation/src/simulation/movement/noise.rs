...
/// # Example
/// ```ignore
/// let noise_x = perlin_locomotion_noise(entity_id, tick, 0);
/// let noise_y = perlin_locomotion_noise(entity_id, tick, 1);
/// ```
pub fn perlin_locomotion_noise(entity_id: u32, tick: u64, axis: u8) -> f32 {
    ...
    let time_scale = 0.05; // Controls wobble frequency (slower = smoother)
    let spatial_offset = (entity_id as f64) * 100.0; // Spatial separation between creatures
    let t = (tick as f64) * time_scale + spatial_offset;
...
mod tests {
    ...
    fn test_noise_in_valid_range() {
        for tick in 0..100 {
            let noise = perlin_locomotion_noise(1, tick, 0);
    ...
    fn test_noise_deterministic() {
        ...
        let noise1 = perlin_locomotion_noise(42, 100, 0);
        let noise2 = perlin_locomotion_noise(42, 100, 0);
    ...
    fn test_noise_varies_with_tick() {
        ...
        let noises: Vec<f32> = (0..10).map(|t| perlin_locomotion_noise(1, t * 10, 0)).collect();
    ...
    fn test_noise_varies_with_entity() {
        ...
        let noises: Vec<f32> = (1..=10).map(|e| perlin_locomotion_noise(e, 50, 0)).collect();
    ...
    fn test_noise_independent_axes() {
        ...
        let noises_x: Vec<f32> = (0..10).map(|t| perlin_locomotion_noise(1, t, 0)).collect();
        let noises_y: Vec<f32> = (0..10).map(|t| perlin_locomotion_noise(1, t, 1)).collect();
