...
/// World configuration at time of snapshot
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Simulation {
    ...
    pub fn to_snapshot(&mut self) -> WorldSnapshot {
        ...
        for (entity, position, velocity, acceleration, rotation, creature_state, wander_state, flee_state) in query_state.iter(&self.world) {
    ...
    pub fn from_snapshot(snapshot: WorldSnapshot) -> Self {
        ...
        for creature in snapshot.creatures {
            ...
            simulation.entity_id_map.insert(entity, creature.id);
        }
...
mod tests {
    use super::*;
    use crate::spawner::{spawn_creature, CreatureSpawnRequest};
    ...
    fn test_simulation_snapshot_empty() {
        ...
        assert_eq!(snapshot.world.width, 100.0);
        assert_eq!(snapshot.world.height, 100.0);
    }
    ...
    fn test_simulation_restore_from_snapshot() {
        ...
        let (width, height) = restored.get_boundaries();
    ...
    fn test_simulation_snapshot_preserves_state() {
        ...
        assert_eq!(creature.position.y, 75.0);
    ...
    fn test_simulation_full_cycle_save_load() {
        ...
        // Report file size
        let metadata = fs::metadata(&path).unwrap();
        println!("Snapshot file size for 100 creatures: {} bytes", metadata.len());
