...
|----------|-----------|--------|
| **🎉 ACHIEVED — Windows** | **1,000,000 creatures @ 20 Hz** | The art of the possible, *reached* (2026-06-21). **48.5 ms mean / 49.4 ms p99 / 49.4 ms max — the whole distribution under 50 ms — 0 render stall frames, ~2.8 GB.** Config: realistic (log-normal) DNA, perception/behavior divisor 8, full ±5000 world. Evidence: [`win_pop1M_48.5ms`](../performance/snapshots/win_pop1M_48.5ms_2026-06-21_2333.json). The deterministic [latency lab](./latency-tuning-lab.md) cross-checks within ~5% per-phase. **At the wall** — lab multi-seed p99 ~51 ms, no headroom; cold-start (indefinite-sustained-with-churn needs the [f32 id-precision fix](../testing/bugs/f32-id-precision-ceiling.md)). **Not yet CI-gated.** |
| **What crossed the line** | **biology, not engine-polish** | Switching random DNA from uniform (~half giants) to a realistic log-normal size pyramid cut perception ~14.5 → 9.1 ms (`cellsQueried` −47%) — the slice that crossed 50 ms. The incremental grid and perception micro-ops didn't get there; one biology decision did. |
