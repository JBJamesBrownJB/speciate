...
> ℹ️ This run executed on the pre-fix workflow (primes-only logging); the four `CLOUD_TRIED` rows were back-filled from this table. Every subsequent run logs them automatically — see `.claude/workflows/perf-hunt-cloud.mjs` (Log phase) and `buildTriedLine` in `.claude/workflows/lib/cloud-ledger.mjs`.
